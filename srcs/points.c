...
static void	set_point_values(t_env *env, size_t x, size_t y, int z)
{
	t_point	*point;
	...
	if (point->source_z > env->max_z)
		env->max_z = point->source_z;
	if (point->source_z < env->min_z)
		env->min_z = point->source_z;
}

bool	parse_point_input(char *point_input, t_env *env, size_t x, size_t y)
{
	t_atoi_res	atoi_res;
	...
	set_point_values(env, x, y, atoi_res.num);
	if (atoi_res.endptr[0] == '\0')
	{
		set_default_color(&env->points[y][x]);
		return (true);
	}
	...
	set_rgb_color(&env->points[y][x], atoi_res.endptr + 3);
	return (true);
}
